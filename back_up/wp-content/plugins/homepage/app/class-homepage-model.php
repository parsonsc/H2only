...
class Homepage_Model {
	...
	function get_upload_path() {
		...
		return $uploads['basedir'] . '/homepage/';
	}
	
	function get_upload_url() {
	
		$uploads = wp_upload_dir();
		return $uploads['baseurl'] . '/homepage/';
	}
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			// div data tag
			$ret .= '<ul class="homepageslider" id="homepageslider-' . $id . '" data-sliderid="' . $id . '"';
            ...
            $ret .= ' data-jsfolder="' . HOMEPAGE_URL . 'engine/"';
			*/
			$ret .= '>';
			...
			if (isset($data->slides) && count($data->slides) > 0)
			{
				...
				{		
                    ...
                    $xtra = ((int)$slide->type == 4)? ' news_box':'';
					$ret .= '<li class="'. $slide->boxsize . $xtra .'">';
					if ((int)$slide->type == 4){
                        $ret .= '<img src="' .  HOMEPAGE_URL . 'engine/news_panel.png"';
                        $ret .= ' alt="'.$this->eacape_html_quotes($slide->description).'"';
                        $ret .= ' />';
                    }
					if ($slide->lightbox)
					{
					...
					else if ($slide->weblink && strlen($slide->weblink) > 0 && $slide->type != 4)
					{
					...
					if ($slide->type == 4){
                        $content = $this->eacape_html_quotes($slide->description);
                        $content = apply_filters('the_content', $content);
                        $content = str_replace(']]>', ']]&gt;', $content);
                        $ret .= '<div class="news_content">'.$content;
                    }
                    else {

                        $ret .= '<figure class="hover"><img src="' . $slide->image . '"';
                        $ret .= ' alt="' .  $this->eacape_html_quotes($slide->title) . '"';
                        $ret .= ' data-description="' .  $this->eacape_html_quotes($slide->description) . '"';
                        $ret .= ' /><figcaption>
                        <h3>' .  $this->eacape_html_quotes($slide->title) . '</h3>
                        <p>' .  $this->eacape_html_quotes($slide->description) . '</p></figcaption></figure>';
                    }
					if ($slide->lightbox || ($slide->weblink && strlen($slide->weblink) > 0 && $slide->type != 4) )
					{
						$ret .= '</a>';
					}					
					if (!$slide->lightbox)
					...
					if ($slide->button && strlen($slide->button) > 0)
					{
						if ($slide->buttonlink && strlen($slide->buttonlink) > 0)
						{
							...
							if ($slide->buttoncss && strlen($slide->buttoncss) > 0)
								$ret .= ' class="' . $slide->buttoncss . '"';                                
							$ret .= '>';
						}
	...
	function save_item($item) {
		...
		if ( ($id > 0) && $this->is_id_exist($id) )
		{
			...
			if (!$ret)
			{
				return array(
						"success" => false,
						"id" => $id, 
						"message" => "Cannot update the homepage blocks in database"
					);
			}
		}
		else
		{
			...
			if (!$ret)
			{
				return array(
						"success" => false,
						"id" => -1,
						"message" => "Cannot insert the homepage blocks to database"
				);
			}
		...
		return array(
				"success" => true,
				"id" => intval($id),
				"message" => "Homepage blocks published!"
		);
	}
	...
	function get_userrole() {
		...
		$userrole = get_option( 'homepage_userrole' );
		if ( $userrole == false )
		{
			update_option( 'homepage_userrole', 'manage_options' );
	...
	function save_settings($options) {
		...
		if (!isset($options) || !isset($options['userrole']))
			...
			$userrole = 'manage_options';
		update_option( 'homepage_userrole', $userrole );
	}
}
